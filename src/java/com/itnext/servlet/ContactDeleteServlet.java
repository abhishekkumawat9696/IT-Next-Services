...
import com.itnext.dao.ContactDao;
import com.itnext.dao.EmployeeDao;
...
public class ContactDeleteServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int eid=Integer.parseInt(request.getParameter("cid"));
            Contact con=new Contact();
            con.setCId(eid);
            
             ContactDao dao=new ContactDao(connectionProvider.getConnection());
                boolean ans=dao.deleteContact(con);
                if(ans)
                {
                    out.println("done");
                }
                ...
                response.sendRedirect("con_requests.jsp");
